...
export function GlucoseProvider({ children }: { children: React.ReactNode }) {
  const { account, isSignedIn, isLoading: authLoading, caregiverSession, caregiverCloudCode, caregiverCodeKind, viewingPatientId, nurseViewCode, profile, doseSyncNonce } = useAuth();
  ...
  /** Bumped when a settings loader finishes, so the server-adoption effect below re-applies after it. */
  const [settingsLoadTick, setSettingsLoadTick] = useState(0);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (settings) {
        ...
        // Placeholders only until the profile's values are known — the adoption effect re-applies next.
      }
      setSettingsLoadTick((t) => t + 1);
    })();
    return () => {
      cancelled = true;
    };
  ...
  /**
   * THE SERVER OWNS THE DOSE SETTINGS; this device only caches them.
   *
   * Whenever the profile's dose values are known — the owner's own row, the circle owner's values
   * for a linked co-guardian (resolved server-side), or the viewed patient's for a code / nurse /
   * linked-patient session — they become the live dose math here, and (outside a transient viewing
   * session) the device cache, so an offline cold start shows the last known server values.
   *
   * This replaces three separate mechanisms that each moved values between the device and the
   * server and between them produced every "my ratios reset on their own" report:
   *  - a launch-time BACKFILL that pushed this device's saved values over the server's whenever they
   *    differed — so a second device, or any change made elsewhere, was silently reverted;
   *  - a one-shot SEED that a slower settings loader could overwrite with 15/120/50 placeholders;
   *  - a member-only OVERLAY, outside of which a co-guardian ran on their own row's stale defaults.
   * Nothing here ever writes to the server. Edits go through `updateProfile` → `setDoseSettings`.
   */
  const pCarbRatio = profile?.carbRatio;
  const pTargetGlucose = profile?.targetGlucose;
  const pCorrectionFactor = profile?.correctionFactor;
  const pByTimeKey = canonicalDoseSettingsByTime(profile?.doseSettingsByTime);
  useEffect(() => {
    if (authLoading || isLoading || !profile) return;
    const known =
      typeof pCarbRatio === "number" || typeof pTargetGlucose === "number" || typeof pCorrectionFactor === "number";
    if (!known) return; // the server holds no dose values (e.g. a nurse account) — leave the cache be
    if (typeof pCarbRatio === "number") setCarbRatioState(pCarbRatio);
    if (typeof pTargetGlucose === "number") setTargetGlucoseState(pTargetGlucose);
    if (typeof pCorrectionFactor === "number") setCorrectionFactorState(pCorrectionFactor);
    const byTime = normalizeDoseSettingsByTime(profile.doseSettingsByTime);
    setDoseSettingsByTimeState(byTime);
    // Someone else's values (code / nurse / linked-patient view) are never cached as this device's own.
    const transientView = !!(caregiverSession && caregiverCloudCode) || !!viewingPatientId || !!nurseViewCode;
    if (!transientView) {
      void mergeSettings({
        ...(typeof pCarbRatio === "number" ? { carbRatio: pCarbRatio } : {}),
        ...(typeof pTargetGlucose === "number" ? { targetGlucose: pTargetGlucose } : {}),
        ...(typeof pCorrectionFactor === "number" ? { correctionFactor: pCorrectionFactor } : {}),
        doseSettingsByTime: byTime ?? null,
      });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- keyed on the VALUES, not profile identity
  }, [
    authLoading, isLoading, settingsLoadTick, doseSyncNonce, pCarbRatio, pTargetGlucose, pCorrectionFactor, pByTimeKey,
    caregiverSession, caregiverCloudCode, viewingPatientId, nurseViewCode,
  ]);
