...
/**
 * Mint the circle's doctor code from a CO-GUARDIAN device — create-only, never replace.
 *
 * The doctor code is a bearer credential that grants dose-editing rights, so this is deliberately the
 * narrowest possible grant: a member may fill an EMPTY slot so a doctor can be onboarded without
 * waiting on the owner, but may never rotate or invalidate a code that already exists. Only the owner
 * can do that (there is no member path to it at all), so nobody but the owner can cut a doctor off
 * mid-treatment, and a member cannot silently swap a code the owner already handed out.
 *
 * Owners do NOT use this — they write their own row directly through the normal profile commit.
 *
 * Returns the code that is now active. If one already exists this returns it UNCHANGED rather than
 * throwing, so a racing second member simply converges on the winner's code.
 */
export const createDoctorCodeAsMember = mutation({
  args: { ...legacyAuthArgs, code: v.string() },
  handler: async (ctx, args) => {
    const user = await requireUserCompat(ctx, args);
    const { anchor, isOwner } = await circleAnchorFor(ctx, user._id);
    if (isOwner) throw new ConvexError("Owners generate the doctor code on their own profile");
    const code = args.code.trim().toUpperCase();
    if (!/^[A-Z0-9]{6,10}$/.test(code)) throw new ConvexError("Invalid doctor code");
    const row = await ctx.db
      .query("patientProfiles")
      .withIndex("by_userId", (q) => q.eq("userId", anchor))
      .unique();
    if (!row) throw new ConvexError("The circle owner hasn't finished setting up their profile");
    // Create-only. An existing code is returned as-is; replacing it is owner-only.
    if (row.doctorCode) return { code: row.doctorCode, created: false };
    const now = new Date().toISOString();
    const memberName = await displayNameFor(ctx, user._id);
    const entry = {
      id: `log_${Date.now()}`,
      timestamp: now,
      action: `Doctor code generated by co-guardian ${memberName}`,
      actor: "caregiver" as const,
    };
    await ctx.db.patch(row._id, {
      doctorCode: code,
      doctorCodeIssuedAt: now,
      // The owner must be able to see that someone else minted their credential.
      accessLog: [...(row.accessLog ?? []), entry].slice(-50),
      updatedAt: Date.now(),
    });
